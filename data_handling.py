...
class PhotometryData:
    ...
    def add_mask(self, name, mask):
        ...
        if len(mask) != self._total_objects:
            raise ValueError(
                f"Mask length ({len(mask)}) does not match data length ({self._total_objects})"
            )
        ...
        for name in names:
            if name == "x":
                ...
                if len(self._data[self._current_filter]) != len(current_mask):
                    raise ValueError(
                        f"Filter data length ({len(self._data[self._current_filter])}) "
                ...
                if len(self._data[name]) != len(current_mask):
                    raise ValueError(
                        f"Data array '{name}' length ({len(self._data[name])}) "
                        f"does not match mask length ({len(current_mask)})"
                    )
    ...
    def apply_mask(self, mask, name=None):
        ...
        if len(mask) != self._total_objects:
            raise ValueError(
                f"New mask length ({len(mask)}) does not match data length ({self._total_objects})"
            )
...
def make_pairs_to_fit(det, cat, nearest_ind, imgwcs, options, data, maglim=None):
    ...
    try:
        ...
        if det.meta["PHFILTER"] in filter_mags:
            ...
            raise ValueError(
                f"Filter '{det.meta['PHFILTER']}' not found in catalog data"
            )
        ...
        logging.error(
            f"Error in make_pairs_to_fit: Missing key in detection or catalog data: {e}"
        )
