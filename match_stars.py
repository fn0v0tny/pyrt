...
def match_stars(det, cat, imgwcs, idlimit=2.0):
    ...
    nearest_ind, _ = tree.query_radius(
        det_coords, r=idlimit, return_distance=True, count_only=False
    )
...
def find_target(det, imgwcs, idlimit=2.0):
    ...
    try:
        ...
        target_x, target_y = imgwcs.all_world2pix(
            [det.meta["OBJRA"]], [det.meta["OBJDEC"]], 1
        )
        ...
        object_ind, object_dist = tree.query_radius(
            target_coords, r=idlimit, return_distance=True, count_only=False
        )
        ...
        if len(object_ind[0]) > 0:
            ...
            if target_match is not None:
                logging.info(
                    f"Target is object id {target_match['NUMBER']} at distance {mindist:.2f} px"
                )
                return target_match
...
def estimate_rough_zeropoint(det, nearest_ind, cat, valid_cat_mask):
    ...
    if np.sum(bright_mask) > 0:
        ...
        logging.info(
            f"Initial zeropoint estimate: {zp:.3f} using {np.sum(bright_mask)} bright stars"
        )
        return zp
...
def get_catalog_with_dynamic_limit(det, estimated_zp, options):
    ...
    logging.info(
        f"Detection limit: {detection_limit:.2f}, using catalog limit: {maglim:.2f}"
    )
...
def process_image_with_dynamic_limits(det, options):
    ...
    try:
        ...
        target_match = find_target(
            det, imgwcs, idlimit=options.idlimit if options.idlimit else 2.0
        )
        ...
        galactic_lat = abs(
            det.meta.get("GLAT", 45.0)
        )  # default to mid-latitude if not available
        ...
        cat = Catalog(
            ...
            catalog="makak"
            if options.makak
            else (options.catalog or "atlas@localhost"),
        )
        ...
        if options.maglim is None:
            ...
            # Get catalog with calculated magnitude limit
            catalog_name = (
                "makak" if options.makak else (options.catalog or "atlas@localhost")
            )
