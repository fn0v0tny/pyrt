...
from file_utils import try_ecsv, try_img, try_sex
...
def read_options(args=sys.argv[1:]):
    ...
    parser.add_argument(
        "-f", "--filter", help="Override filter info from fits", type=str
    )
    parser.add_argument(
...
def fix_time(hdr, verbose=False):
    ...
    with suppress(KeyError):
        do_ctime = Time(hdr["DATE-OBS"]).to_value("unix")
    if verbose and np.isnan(do_ctime):
        print("No DATE-OBS? That's BAD!")

    with suppress(KeyError):
        jd_ctime = (hdr["JD_START"] - 2440587.5) * 86400
    with suppress(KeyError):
        jd_ctime = (hdr["JD"] - 2440587.5) * 86400
    with suppress(KeyError):
        mjd_ctime = (hdr["MJD"] - 40587.0) * 86400
    with suppress(KeyError):
        mjd_ctime = (hdr["MJD-OBS"] - 40587.0) * 86400
    usec = 0
    with suppress(KeyError):
        usec = hdr["USEC"]
    with suppress(KeyError):
        ux_ctime = np.float64(hdr["CTIME"] + usec / 1000000.0)
    ...
    if not np.isnan(ux_jd) and ux_jd > 0.001 and verbose:
        print(f"ux/jd  time info differ by {ux_jd}s!")
    if not np.isnan(ux_mjd) and ux_mjd > 0.001 and verbose:
        print(f"ux/mjd time info differ by {ux_mjd}s!")
    if not np.isnan(ux_do) and ux_do > 0.001 and verbose:
        print(f"ux/do  time info differ by {ux_do}s!")
    if not np.isnan(jd_mjd) and jd_mjd > 0.001 and verbose:
        print(f"jd/mjd time info differ by {jd_mjd}s!")
    if not np.isnan(jd_do) and jd_do > 0.001 and verbose:
        print(f"jd/do  time info differ by {jd_do}s!")
    if not np.isnan(mjd_do) and mjd_do > 0.001 and verbose:
        print(f"mjd/do time info differ by {mjd_do}s!")

    ctime = np.nan
    if not np.isnan(ux_ctime) and np.isnan(ctime):
        if verbose:
            print(f"JD set based on CTIME+USEC={ux_ctime:.6f}")
        ctime = ux_ctime
    if not np.isnan(do_ctime) and np.isnan(ctime):
        if verbose:
            print(f"JD set based on DATE-OBS={do_ctime:.6f}")
        ctime = do_ctime
    if not np.isnan(mjd_ctime) and np.isnan(ctime):
        if verbose:
            print(f"JD set based on MJD-OBS={do_ctime:.6f}")
        ctime = mjd_ctime
    if not np.isnan(jd_ctime) and np.isnan(ctime):
        if verbose:
            print(f"JD set based on the original JD={do_ctime:.6f}")
        ctime = jd_ctime
    ...
    if not np.isnan(ctime):
        hdr["JD"] = 2440587.5 + ctime / 86400.0
...
def get_limits(det, verbose=False):
    ...
    res = opt.least_squares(
        errormodel,
        [1, 2.5],
        args=[(det["MAG_AUTO"], np.log10(det["MAGERR_AUTO"] * np.log(2.5) * 3))],
    )
    ...
    if verbose and res.success:
        print(
            f"Limits fitted, LIMFLX3 = {det.meta['LIMFLX3']:.3f} +/- {det.meta['DLIMFLX3']:.3f}"
        )
...
def open_files(arg, verbose=False):
    ...
    if img is None:
        imgf, img = try_img(os.path.splitext(arg)[0], verbose)
    if img is None:
        imgf, img = try_img(os.path.splitext(arg)[0] + ".fits", verbose)
    if det is None:
        detf, det = try_sex(arg + ".xat", verbose)
    if det is None:
    ...
    if os.path.getmtime(imgf) > os.path.getmtime(detf):
        if verbose:
            print(
                f"Warning: {imgf} is {os.path.getmtime(imgf)-os.path.getmtime(detf):.0f}s newer than {detf}!"
            )
...
def fix_latlon(hdr, verbose=False):
    ...
    with suppress(KeyError):
        latitude = np.float64(hdr["TEL_LAT"])
    with suppress(KeyError):
        latitude = np.float64(hdr["LATITUDE"])
    hdr["LATITUDE"] = latitude

    longitude = 14.7819092  # BART Longitude
    with suppress(KeyError):
        longitude = np.float64(hdr["TEL_LONG"])
    with suppress(KeyError):
        longitude = np.float64(hdr["LONGITUD"])
    hdr["LONGITUD"] = longitude

    altitude = 530  # BART Altitude
    with suppress(KeyError):
        altitude = np.float64(hdr["TEL_ALT"])
    with suppress(KeyError):
        altitude = np.float64(hdr["ALTITUDE"])
    hdr["ALTITUDE"] = altitude

    if verbose:
        print(
            f"Observatory at {hdr['LONGITUD']:.3f},{hdr['LATITUDE']:.3f} at {hdr['ALTITUDE']:.0f}m "
        )
...
def fix_filter(hdr, verbose=False, opt_filter=None):
    ...
    if verbose:
        print(f"Filter changed from {old_fltr} to {fits_fltr}")
...
def main():
    ...
    for arg in options.files:
        ...
        for i, j in c.items():
            if i in ("NAXIS", "NAXIS1", "NAXIS2", "BITPIX"):
                continue
            if len(i) > 8:
                continue
            if "." in i:
        ...
        if options.verbose:
            print(
                f"Target is {det.meta['OBJECT']} at ra:{det.meta['OBJRA']} dec:{det.meta['OBJDEC']} -> x:{tmp[0]} y:{tmp[1]}"
            )
        ...
        rd = imgwcs.all_pix2world(
            [
                [det.meta["CTRX"], det.meta["CTRY"]],
                [0, 0],
                [det.meta["CTRX"], det.meta["CTRY"] + 1],
            ],
            0,
        )
        ...
        center = SkyCoord(
            rd[0][0] * astropy.units.deg, rd[0][1] * astropy.units.deg, frame="fk5"
        )
        corner = SkyCoord(
            rd[1][0] * astropy.units.deg, rd[1][1] * astropy.units.deg, frame="fk5"
        )
        pixel = SkyCoord(
            rd[2][0] * astropy.units.deg, rd[2][1] * astropy.units.deg, frame="fk5"
        ).separation(center)
